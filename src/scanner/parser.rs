fn parse_tag<'a>(content: &'a String, tag: &str) -> Option<&'a str> {
    ...
    let tag_end_index = tag_start_index + content[tag_start_index..].find(tag_end.as_str())? - 1; // -1 -> "<" of </tag>
...
pub fn parse_attribute<'a>(content: &'a str, attr: &str) -> Option<&'a str> {
    ...
    let attribute_content_closing = attribute_content_start + content[attribute_content_start..].find("\"")? - 1; // -1 -> the found "
